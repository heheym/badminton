...
Class Club extends Common
{
    public function index()
    {
        ...
        $data = Db::name('club')->field(["*","concat(province,',',city,',',area) as address"])->paginate(10);
    ...
    public function checkStatus()
    {
        $id = input('id/d');
        $action = input('action');
        $checkContent = input('checkContent','');

        $result = Db::name('club')->where('id',$id)->update(['checkStatus'=>$action,'checkContent'=>$checkContent,'checkTime'=>date('Y-m-d H:i:s')]);

        if($result!==false){
            return success('保存成功',url('admin/club/index'));
        }
        return error('保存失败');
    }
    ...
    public function del()
    {
        ...
        $result= Db::name('community')->where('id',$id)->update(['status'=>0]);
        if($result){
            ...
            if($user){
                ...
                foreach($user as $k =>$v){
                    $html.=<<<EOF
                            <div class="col-sm-1 " style="margin: 10px;" >
                                    <div class="thumbnail" style="width:100px;height:100px;">
                                            <img src="{$v['head']}"  onclick="chang(this)" uid="{$v['id']}">
                                        <div class="caption">
                                            <p>{$v['communiter']}</p>                                   
                                        </div>
                                    </div>
