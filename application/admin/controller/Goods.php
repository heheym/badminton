...
class Goods extends Common
{
        ...
        if($id){
            ...
            if(!empty($list)){
                foreach ($list as $key => $value) {
                    $shelf = $value['status']==0?'上架':'下架'; //edit by ma 上下架
                    $lists[$key] = $value;
                    $lists[$key]['img'] = $value['img'] ? "<img src='".$uploads_tem.$value['img']."' alt='".$value['name']."' width='36'>" : '';
                    $lists[$key]['operate'] = "
                    <a name='post' href='". url('shelf',['gid'=>$value['id'],'id'=>input('id')]) ."' class='btn btn-primary btn-xs'>".$shelf."</a>&nbsp;&nbsp;
                    <a href='". url('edit',['id'=>$value['id']]) ."' title='编辑' target='_parent'><i class='fa fa-edit text-navy'></i></a>&nbsp;&nbsp;
    ...
    public function shelf()
    {
        $cid = input('id');
        $id = input('gid/d');
        $exists = Db::name('goods')->find($id);
        if(!$exists){
            return error('参数错误');
        }
        if($exists['status']==1){
            $update = Db::name('goods')->update(['id'=>$id,'status'=>0]);
            if($update){
                return success('下架成功',url('goods/content',['id'=>$cid]));
            }
            return error('下架失败',url('goods/content',['id'=>$cid]));
        }
        $update = Db::name('goods')->update(['id'=>$id,'status'=>1]);
        if($update){
            return success('上架成功',url('goods/content',['id'=>$cid]));
        }
        return error('上架失败',url('goods/content',['id'=>$cid]));
    }
}
